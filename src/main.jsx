...
function CaptureBar({ onCreate, onOpenCreate }) { const [title, setTitle] = useState(''); const submit = async (event) => { event.preventDefault(); if (await onCreate({ title, status: '收件箱' })) setTitle(''); }; return <form className="capture-bar" onSubmit={submit}><Plus size={17} /><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="想到什么？直接记下来，按 Enter 保存" aria-label="快速记录任务" /><button type="button" className="secondary capture-more" onClick={onOpenCreate}>补充信息</button><button className="primary" type="submit" disabled={!title.trim()}>记录</button></form>; }
