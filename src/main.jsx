...
function App() {
  ...
  const createTask = async ({ title, description = '', projectId = projectFilter, parentId = null, status = '收件箱', importance = false, urgency = false, scheduledFor = null, dueAt = null }) => {
    const trimmed = title.trim(); if (!trimmed) return null;
    try {
      const result = await api('tasks', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ title: trimmed, description: description.trim(), status, project_id: projectId || null, parent_id: parentId, importance, urgency, scheduled_for: scheduledFor || null, due_at: dueAt ? new Date(`${dueAt}:00+08:00`).toISOString() : null, manual_order: Date.now() }) });
