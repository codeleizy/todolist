...
function App() {
  ...
  const createTask = async ({ title, description = '', projectId = projectFilter, parentId = null, status = '收件箱' }) => {
    const trimmed = title.trim(); if (!trimmed) return null;
    try {
      const result = await api('tasks', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ title: trimmed, description: description.trim(), status, project_id: projectId || null, parent_id: parentId, manual_order: Date.now() }) });
...
function ViewContent({ view, mode, tasks, month, setMonth, onSelect, onComplete, onUpdate, onDelete, onCreate, childrenByParent, projects, categories, onQuickCreate }) {
  ...
  const unscheduled = working.filter((task) => !task.due_at && !task.scheduled_for && task.status !== '进行中');
  return <TodayWorkspace late={late} inProgress={inProgress} planned={planned} future={future} unscheduled={unscheduled} onSelect={onSelect} onComplete={onComplete} onUpdate={onUpdate} childrenByParent={childrenByParent} projects={projects} categories={categories} />;
}
function TodayWorkspace({ late, inProgress, planned, future, unscheduled, ...props }) { const groups = [{ title: '需要处理', hint: '逾期 / 今天截止', tasks: late, tone: 'attention' }, { title: '正在推进', hint: '进行中的任务', tasks: inProgress, tone: 'progress' }, { title: '今日计划', hint: '今天安排的任务', tasks: planned, tone: 'plan' }]; return <div className="today-workspace"><div className="today-summary">{groups.map((group) => <div key={group.title} className={`today-metric ${group.tone}`}><span>{group.title}</span><strong>{group.tasks.length}</strong><small>{group.hint}</small></div>)}</div><div className="today-lanes">{groups.map((group) => <section key={group.title} className={`today-lane ${group.tone}`}><header><div><strong>{group.title}</strong><span>{group.hint}</span></div><small>{group.tasks.length}</small></header><TaskList tasks={group.tasks} {...props} empty="暂无任务" /></section>)}</div>{unscheduled.length > 0 && <section className="today-upcoming"><header><strong>待安排</strong><span>尚未设置计划或截止日期的任务</span></header><TaskList tasks={unscheduled.slice(0, 6)} {...props} /></section>}{future.length > 0 && <section className="today-upcoming"><header><strong>接下来截止</strong><span>未来有明确截止时间的任务</span></header><TaskList tasks={future.slice(0, 6)} {...props} /></section>}</div>; }
