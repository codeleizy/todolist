...
function App() {
  ...
  return <div className="shell">
    <aside className="sidebar">
      <div className="brand"><b>✓</b><span>一掐四</span></div>
      <button className="primary" onClick={() => setModal('task')}><Icon>＋</Icon>新建任务</button>
      <nav className="nav" aria-label="主导航">
        <NavButton icon="▣" label="收件箱" active={!projectFilter && view === 'inbox'} onClick={() => changeView('inbox')} count={tasks.filter((item) => active(item) && item.status === '收件箱' && !item.parent_id).length} />
        <NavButton icon="◷" label="今日" active={!projectFilter && view === 'today'} onClick={() => changeView('today')} />
        <NavButton icon="⊞" label="四象限" active={!projectFilter && view === 'quadrant'} onClick={() => changeView('quadrant')} />
        <NavButton icon="▦" label="日历" active={!projectFilter && view === 'calendar'} onClick={() => changeView('calendar')} />
        <NavButton icon="⌑" label="归档" active={!projectFilter && view === 'archive'} onClick={() => changeView('archive')} />
      </nav>
      <div className="side-heading"><span>项目</span><button aria-label="新建项目" onClick={() => setModal('project')}>＋</button></div>
      ...
      <button className="new-category" onClick={() => projects.length ? setModal('category') : flash('请先创建项目')}>＋ 新建分类</button>
    </aside>

    <main className="main">
      <header className="topbar">
        <div><h1>{title}</h1><p>{subtitle}</p></div>
        <div className="toolbar"><label className="search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索全部任务" /></label><button className="secondary" onClick={load}>↻ 刷新</button></div>
      </header>
      ...
      <section className="content" aria-busy={loading}>
        {loading ? <div className="empty">正在载入任务…</div> : projectFilter ? <TaskList tasks={visibleTasks.filter(active)} onSelect={setSelected} onComplete={completeTask} childrenByParent={childrenByParent} empty="这个项目还没有未完成任务" /> : <ViewContent view={view} tasks={visibleTasks} month={month} setMonth={setMonth} onSelect={setSelected} onComplete={completeTask} childrenByParent={childrenByParent} />}
      </section>
    </main>
    ...
    <nav className="mobile-nav" aria-label="移动导航">
      <button onClick={() => changeView('inbox')}>收件箱</button><button onClick={() => changeView('today')}>今日</button><button onClick={() => setModal('task')} className="mobile-add">＋</button><button onClick={() => changeView('quadrant')}>四象限</button><button onClick={() => changeView('calendar')}>日历</button>
    </nav>
...
function NavButton({ icon, label, active, onClick, count }) { return <button className={active ? 'active' : ''} onClick={onClick}><Icon>{icon}</Icon><span>{label}</span>{count !== undefined && <small>{count}</small>}</button>; }

function ViewContent({ view, tasks, month, setMonth, onSelect, onComplete, childrenByParent }) {
  const root = tasks.filter((task) => !task.parent_id);
  if (view === 'inbox') return <TaskList tasks={root.filter((task) => active(task) && task.status === '收件箱')} onSelect={onSelect} onComplete={onComplete} childrenByParent={childrenByParent} empty="收件箱已经清空" />;
  if (view === 'archive') return <TaskList tasks={root.filter((task) => task.archived_at || ['已完成', '已取消'].includes(task.status))} onSelect={onSelect} onComplete={onComplete} childrenByParent={childrenByParent} empty="还没有归档记录" />;
  ...
  return <div className="today-grid"><Section title="已逾期 / 今天截止" tasks={late} onSelect={onSelect} onComplete={onComplete} childrenByParent={childrenByParent} /><Section title="进行中" tasks={inProgress} onSelect={onSelect} onComplete={onComplete} childrenByParent={childrenByParent} /><Section title="计划今天" tasks={planned} onSelect={onSelect} onComplete={onComplete} childrenByParent={childrenByParent} /><Section title="未来有截止时间" hint="所有未来截止任务都会列出" tasks={future} onSelect={onSelect} onComplete={onComplete} childrenByParent={childrenByParent} /></div>;
}
...
function TaskList({ tasks, onSelect, onComplete, childrenByParent, empty = '暂无任务' }) { return <div className="task-list">{tasks.length ? tasks.map((task) => <TaskRow key={task.id} task={task} onSelect={onSelect} onComplete={onComplete} children={childrenByParent.get(task.id) || []} />) : <div className="empty">{empty}</div>}</div>; }

function TaskRow({ task, onSelect, onComplete, children }) {
  const due = dateOnly(task.due_at); const isLate = due && due < today();
  return <article className="task-row" tabIndex="0" onClick={() => onSelect(task)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onSelect(task); } }}>
    <button className={task.status === '已完成' ? 'check done' : 'check'} aria-label={`${task.status === '已完成' ? '恢复' : '完成'}：${task.title}`} onClick={(event) => { event.stopPropagation(); onComplete(task); }}>{task.status === '已完成' ? '✓' : ''}</button>
    <div className="task-copy"><strong>{esc(task.title)}</strong><div className="task-meta"><span>{task.status}</span>{children.length > 0 && <span className="child-count">◌ {children.filter((item) => item.status === '已完成').length}/{children.length} 子任务</span>}</div></div>
...
function Calendar({ tasks, month, setMonth, onSelect }) {
  ...
  return <section className="calendar"><div className="calendar-head"><button aria-label="上个月" onClick={() => setMonth(new Date(year, monthIndex - 1, 1))}>‹</button><strong>{year} 年 {monthIndex + 1} 月</strong><button aria-label="下个月" onClick={() => setMonth(new Date(year, monthIndex + 1, 1))}>›</button></div><div className="weekdays">{['日', '一', '二', '三', '四', '五', '六'].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{cells.map((day, index) => { if (!day) return <div className="calendar-day blank" key={`b-${index}`} />; const value = dateKey(day); const planned = tasks.filter((task) => task.scheduled_for === value); const due = tasks.filter((task) => dateOnly(task.due_at) === value && task.scheduled_for !== value); const items = [...due.map((task) => ({ task, type: 'due' })), ...planned.map((task) => ({ task, type: 'plan' }))]; return <div className={value === today() ? 'calendar-day today' : 'calendar-day'} key={value}><span>{day}</span>{items.slice(0, 2).map(({ task, type }) => <button key={task.id} className={type} onClick={() => onSelect(task)}>{task.title}</button>)}{items.length > 2 && <em>+{items.length - 2} 项</em>}</div>; })}</div></section>;
}
...
function TaskDetail({ task, projects, categories, childTasks, onSave, onComplete, onClose, onAddChild }) {
  ...
  return <div className="detail"><div className="detail-top"><span>任务详情</span><button aria-label="关闭详情" onClick={onClose}>×</button></div><input className="detail-title" aria-label="任务名称" value={draft.title || ''} onChange={(event) => set('title', event.target.value)} /><label>状态<select value={draft.status} onChange={(event) => set('status', event.target.value)}>{['收件箱', '待进行', '进行中', '已完成', '已取消', '阻塞'].map((item) => <option key={item}>{item}</option>)}</select></label><div className="two-fields"><label>重要度<select value={String(draft.importance)} onChange={(event) => set('importance', event.target.value === 'true')}><option value="true">重要</option><option value="false">不重要</option></select></label><label>紧急度<select value={String(draft.urgency)} onChange={(event) => set('urgency', event.target.value === 'true')}><option value="true">紧急</option><option value="false">不紧急</option></select></label></div><label>描述<textarea value={draft.description || ''} onChange={(event) => set('description', event.target.value)} placeholder="补充说明…" rows="4" /></label><div className="two-fields"><label>计划日期<input type="date" value={draft.scheduled_for || ''} onChange={(event) => set('scheduled_for', event.target.value)} /></label><label>截止时间<input type="datetime-local" value={draft.due_at || ''} onChange={(event) => set('due_at', event.target.value)} /></label></div><label>项目<select value={draft.project_id || ''} onChange={(event) => set('project_id', event.target.value)}><option value="">未设置</option>{projects.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>分类<select value={draft.category_id || ''} onChange={(event) => set('category_id', event.target.value)}><option value="">未设置</option>{categories.filter((item) => !draft.project_id || item.project_id === draft.project_id).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><div className="children-head"><span>子任务 {childTasks.filter((item) => item.status === '已完成').length}/{childTasks.length}</span><button onClick={onAddChild}>＋ 添加</button></div>{childTasks.map((child) => <div className="child" key={child.id}><span className={child.status === '已完成' ? 'child-check done' : 'child-check'}>{child.status === '已完成' ? '✓' : ''}</span>{child.title}</div>)}<div className="detail-actions"><button className="secondary" onClick={() => onComplete(task)}>{task.status === '已完成' ? '恢复任务' : '完成任务'}</button><button className="primary save" onClick={save}>保存更改</button></div></div>;
}
