...
function App() {
  ...
  return <div className="shell">
    ...
    <main className={view === 'inbox' && !projectFilter ? 'main inbox-main' : 'main'}>
      ...
      <CreateTaskSheet open={quickCreate} initialStatus={quickStatus} projectId={projectFilter} projects={projects} categories={categories} onOpenChange={setQuickCreate} onSubmit={async (payload) => { const created = await createTask({ title: payload.title, projectId: payload.projectId, status: payload.status }); if (created && payload.description) await updateTask(created.id, { description: payload.description }, '任务已创建'); return created; }} />
    ...
    <TaskSheet task={selected} projects={projects} categories={categories} childTasks={selected ? childrenByParent.get(selected.id) || [] : []} onClose={() => setSelected(null)} onSave={updateTask} onComplete={completeTask} onDelete={deleteTask} onAddChild={() => selected && createTask({ title: '新的子任务', parentId: selected.id, projectId: selected.project_id })} />
...
function ViewContent({ view, mode, tasks, month, setMonth, onSelect, onComplete, onUpdate, onDelete, onCreate, childrenByParent, projects, categories, onQuickCreate }) {
  ...
  if (view === 'inbox') return <InboxWorkspace tasks={root.filter((task) => active(task) && task.status === '收件箱')} projects={projects} categories={categories} onSelect={onSelect} onComplete={onComplete} onSave={onUpdate} onDelete={onDelete} onAddChild={(task) => onCreate({ title: '新的子任务', parentId: task.id, projectId: task.project_id })} childrenByParent={childrenByParent} />;
...
function TaskSheet({ task, projects, categories, childTasks, onClose, onSave, onComplete, onDelete, onAddChild }) { return <Dialog.Root open={Boolean(task)} onOpenChange={(open) => !open && onClose()}>{task && <Dialog.Portal><Dialog.Overlay className="task-overlay" /><Dialog.Content className="task-sheet" aria-describedby={undefined}><div className="sheet-top"><span>任务详情</span><Dialog.Close className="icon-button" aria-label="关闭详情"><X size={17} /></Dialog.Close></div><Dialog.Title className="sr-only">编辑任务</Dialog.Title><TaskEditor key={task.id} task={task} projects={projects} categories={categories} childTasks={childTasks} onSave={onSave} onComplete={onComplete} onDelete={onDelete} onAddChild={onAddChild} /></Dialog.Content></Dialog.Portal>}</Dialog.Root>; }
function TaskEditor({ task, projects, categories, childTasks, onSave, onComplete, onDelete, onAddChild, autoSave = false, inbox = false }) {
