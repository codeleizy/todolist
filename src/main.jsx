...
import {
  ...
  Clock3, Columns3, Folder, Grid2X2, Inbox, List, ListTodo, MoreHorizontal, Plus,
...
function ViewContent({ view, mode, tasks, month, setMonth, onSelect, onComplete, onUpdate, childrenByParent, projects, categories, onQuickCreate }) {
  ...
  if (view === 'inbox') return <InboxWorkspace tasks={root.filter((task) => active(task) && task.status === '收件箱')} projects={projects} categories={categories} onSelect={onSelect} onComplete={onComplete} onSave={onUpdate} childrenByParent={childrenByParent} />;
...
function InboxWorkspace({ tasks, projects, categories, childrenByParent, onSelect, onComplete, onSave }) {
...
function InboxEditor({ task, projects, categories, childCount, onSave, onOpen, onComplete }) {
  ...
  return <><div className="preview-crumb"><Inbox size={14} />收件箱 / 待处理 <span className={saving ? 'saving-state saving' : 'saving-state'}>{saving ? '正在保存' : saved ? '已保存' : '等待保存'}</span></div><div className="preview-head editor"><div><StatusSwitch value={draft.status} onValueChange={(value) => set('status', value)} /><input className="inbox-title-input" value={draft.title || ''} onChange={(event) => set('title', event.target.value)} aria-label="任务标题" /><textarea className="inbox-description-input" value={draft.description || ''} onChange={(event) => set('description', event.target.value)} placeholder="添加说明、想法或下一步…" rows="6" /></div><button className="icon-button" onClick={() => onOpen(task)} aria-label="打开完整任务编辑"><MoreHorizontal size={18} /></button></div><div className="preview-properties editor-properties"><TreeSelect projects={projects} categories={categories} projectId={draft.project_id} categoryId={draft.category_id} onValueChange={({ projectId, categoryId }) => { setSaved(false); setDraft((current) => ({ ...current, project_id: projectId, category_id: categoryId })); }} /><span>子任务 <b>{childCount} 项</b></span></div><div className="preview-actions"><button className="secondary" onClick={() => onOpen(task)}>完整编辑</button><button className="primary" onClick={() => onComplete(task)}><Check size={15} />完成</button></div></>;
}

function Quadrants({ tasks, onSelect, childrenByParent }) { const data = tasks.map((task, index) => ({ task, x: task.importance ? 65 + (index % 4) * 7 : 12 + (index % 5) * 8, y: task.urgency ? 65 + (Math.floor(index / 4) % 3) * 9 : 12 + (Math.floor(index / 4) % 4) * 9, parent: (childrenByParent.get(task.id) || []).length > 0 })); return <section className="quadrant-chart"><div className="quad-label q-label-urgent">重要且紧急</div><div className="quad-label q-label-plan">重要但不紧急</div><div className="quad-label q-label-delegate">不重要但紧急</div><div className="quad-label q-label-later">不重要且不紧急</div><ResponsiveContainer width="100%" height="100%"><ScatterChart margin={{ top: 28, right: 28, bottom: 28, left: 28 }}><CartesianGrid stroke="#e8e8e5" strokeDasharray="0" /><XAxis type="number" dataKey="x" domain={[0, 100]} hide /><YAxis type="number" dataKey="y" domain={[0, 100]} hide /><Tooltip cursor={false} content={({ active: isActive, payload }) => isActive && payload?.[0] ? <div className="quad-tooltip">{payload[0].payload.task.title}</div> : null} /><Scatter data={data} shape={(props) => <TaskPoint {...props} onSelect={onSelect} />} /></ScatterChart></ResponsiveContainer></section>; }
...
function TaskEditor({ task, projects, categories, childTasks, onSave, onComplete, onAddChild }) {
  const [draft, setDraft] = useState({ ...task, due_at: task.due_at ? task.due_at.slice(0, 16) : '' }); const [saving, setSaving] = useState(false);
  const set = (key, value) => setDraft((item) => ({ ...item, [key]: value }));
  const save = async () => { setSaving(true); const payload = { title: draft.title.trim() || task.title, status: draft.status, importance: draft.importance, urgency: draft.urgency, description: draft.description || '', scheduled_for: draft.scheduled_for || null, due_at: draft.due_at ? new Date(draft.due_at).toISOString() : null, project_id: draft.project_id || null, category_id: draft.category_id || null }; await onSave(task.id, payload, '已保存'); setSaving(false); };
  return <div className="detail"><div className="detail-heading"><input className="detail-title" aria-label="任务名称" value={draft.title || ''} onChange={(event) => set('title', event.target.value)} /><StatusSwitch value={draft.status} onValueChange={(value) => set('status', value)} /></div><textarea className="detail-description" value={draft.description || ''} onChange={(event) => set('description', event.target.value)} placeholder="添加说明…" rows="4" /><section className="detail-section"><h3>归属</h3><TreeSelect projects={projects} categories={categories} projectId={draft.project_id} categoryId={draft.category_id} onValueChange={({ projectId, categoryId }) => setDraft((current) => ({ ...current, project_id: projectId, category_id: categoryId }))} /></section><section className="detail-section"><h3>时间</h3><div className="property-list"><label><span>计划日期</span><input type="date" value={draft.scheduled_for || ''} onChange={(event) => set('scheduled_for', event.target.value)} /></label><label><span>截止时间</span><input type="datetime-local" value={draft.due_at || ''} onChange={(event) => set('due_at', event.target.value)} /></label></div></section><section className="detail-section"><h3>优先级</h3><div className="priority-switches"><PrioritySwitch label="重要" checked={Boolean(draft.importance)} onCheckedChange={(value) => set('importance', value)} /><PrioritySwitch label="紧急" checked={Boolean(draft.urgency)} onCheckedChange={(value) => set('urgency', value)} /></div></section><section className="detail-section children-section"><div className="children-head"><h3>子任务 <em>{childTasks.filter((item) => item.status === '已完成').length}/{childTasks.length}</em></h3><button onClick={onAddChild}><Plus size={14} />添加</button></div>{childTasks.length ? childTasks.map((child) => <div className="child" key={child.id}><span className={child.status === '已完成' ? 'child-check done' : 'child-check'}>{child.status === '已完成' && <Check size={10} strokeWidth={3} />}</span>{child.title}</div>) : <p className="child-empty">拆成小步骤，会更容易推进。</p>}</section><div className="detail-actions"><button className="secondary" onClick={() => onComplete(task)}>{task.status === '已完成' ? '恢复任务' : '完成任务'}</button><button className="primary" onClick={save}>{saving ? '保存中…' : '保存更改'}</button></div></div>;
}
...
function PrioritySwitch({ label, checked, onCheckedChange }) { return <label className="priority-switch"><span>{label}</span><Switch.Root className="radix-switch" checked={checked} onCheckedChange={onCheckedChange}><Switch.Thumb /></Switch.Root></label>; }
